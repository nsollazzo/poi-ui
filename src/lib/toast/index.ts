...
export function toast(message: string, opts: ToastOptions = {}): string {
	// Guard against NaN/Infinity/negative (e.g. a computed duration) silently
	// producing a permanent, countdown-less toast; fall back to the default. `0`
	// (sticky) and non-negative finite values pass through.
	const duration =
		Number.isFinite(opts.duration) && (opts.duration as number) >= 0

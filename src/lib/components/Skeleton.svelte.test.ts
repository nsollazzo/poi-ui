...
describe('Skeleton', () => {
	test('is decorative (aria-hidden) so it is not announced', () => {
		...
		const el = document.querySelector('.pn-skeleton') as HTMLElement;
		expect(el.getAttribute('aria-hidden')).toBe('true');
	});

	test('runs the pulse animation under normal motion', () => {
		render(ThemedHarness, { theme: 'machine', Comp: Skeleton });
		// Svelte scopes @keyframes names (e.g. svelte-xxxx-pn-skeleton-pulse).
		expect(
			getComputedStyle(document.querySelector('.pn-skeleton') as HTMLElement).animationName
		).toContain('pn-skeleton-pulse');
	});

	test('uses the surface-2 lift in The Machine', () => {
		render(ThemedHarness, { theme: 'machine', Comp: Skeleton });
		expect(
			getComputedStyle(document.querySelector('.pn-skeleton') as HTMLElement).backgroundColor
